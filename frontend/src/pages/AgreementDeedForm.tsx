...
import { useGetAllParties, useGetAllLocations, useSaveDraft } from '../hooks/useQueries';
import { generateAgreementDeedText } from '../utils/tamilTextGenerator';
import { numberToTamilWords } from '../utils/tamilNumberConverter';
import type { PartyInfo, WitnessInfo, PreviousDocInfo } from '../utils/tamilTextGenerator';
...
const DEADLINE_UNITS = ['நாள்', 'மாதம்', 'வருடம்'];
...
export default function AgreementDeedForm() {
  const { data: allParties = [] } = useGetAllParties();
  const { data: locations = [] } = useGetAllLocations();
  const saveDraft = useSaveDraft();

  const [step, setStep] = useState(0);
  const [seller, setSeller] = useState<PartyInfo>({ ...emptyParty });
  const [buyer, setBuyer] = useState<PartyInfo>({ ...emptyParty });
  ...
  const [deadlineUnit, setDeadlineUnit] = useState('மாதம்');
  ...
  const [draftId] = useState(`draft_agr_${Date.now()}`);

  const previewText = useMemo(() => {
    if (!seller.name && !buyer.name) return '';
    return generateAgreementDeedText({
      deedDate, buyer, seller, previousDoc: prevDoc,
      totalAmount, advanceAmount, balanceAmount,
      deadline, deadlineUnit, witnesses, preparer, propertyDetails,
    });
  }, [deedDate, buyer, seller, prevDoc, totalAmount, advanceAmount, balanceAmount, deadline, deadlineUnit, witnesses, preparer, propertyDetails]);

  const handleSaveDraft = async () => {
    const formData = JSON.stringify({ deedDate, buyer, seller, prevDoc, totalAmount, advanceAmount, balanceAmount, deadline, deadlineUnit, witnesses, preparer, propertyDetails, step });
    const now = BigInt(Date.now());
    try {
      await saveDraft.mutateAsync({
        id: draftId,
        deedType: 'AgreementDeed' as unknown as DeedType,
        createdAt: now,
        updatedAt: now,
        formData,
      });
  ...
  const totalWords = totalAmount > 0 ? numberToTamilWords(totalAmount) : '';
  const advanceWords = advanceAmount > 0 ? numberToTamilWords(advanceAmount) : '';
  const balanceWords = balanceAmount > 0 ? numberToTamilWords(balanceAmount) : '';

  const sellerAsParty = allParties.find(p => p.name === seller.name);
  const buyerAsParty = allParties.find(p => p.name === buyer.name);
    ...
    <div className="flex h-[calc(100vh-57px)] overflow-hidden">
      ...
      <div className="w-1/2 flex flex-col border-r border-border">
          ...
          <div className="p-4 space-y-4">
            ...
            {step === 0 && (
              <div className="space-y-3">
                ...
                <div className="border border-border rounded-lg p-3 bg-card space-y-2">
                  <div className="text-xs font-semibold text-primary font-tamil">கிரைய உடன்படிக்கை தேதி</div>
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <div>
                      <Label className="text-xs font-tamil">தேதி</Label>
                ...
                <div className="border border-border rounded-lg p-3 bg-card space-y-2">
                  ...
                  <PartySearchSelect
                    parties={allParties}
                    selectedParties={sellerAsParty ? [sellerAsParty] : []}
                    onAdd={p => setSeller(partyToPartyInfo(p))}
                    onRemove={() => setSeller({ ...emptyParty })}
                    label="விற்பவர்"
                  />
                </div>
                <PartyFormFields party={seller} onChange={setSeller} index={0} label="விற்பவர்" />
              </div>
            )}
            ...
            {step === 1 && (
              <div className="space-y-3">
                ...
                <div className="border border-border rounded-lg p-3 bg-card space-y-2">
                  ...
                  <PartySearchSelect
                    parties={allParties}
                    selectedParties={buyerAsParty ? [buyerAsParty] : []}
                    onAdd={p => setBuyer(partyToPartyInfo(p))}
                    onRemove={() => setBuyer({ ...emptyParty })}
                    label="வாங்குபவர்"
                  />
                </div>
                <PartyFormFields party={buyer} onChange={setBuyer} index={0} label="வாங்குபவர்" />
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-3">
                ...
                <div>
                  <Label className="font-tamil text-sm">சொத்து விவரங்கள்</Label>
                  <Textarea
                ...
                {/* Previous Document */}
                <div className="border border-border rounded-lg p-3 bg-card space-y-2">
                  <div className="text-xs font-semibold text-primary font-tamil">முந்தைய ஆவண விவரங்கள்</div>
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <Label className="text-xs font-tamil">தேதி</Label>
            ...
            {step === 3 && (
              <div className="space-y-3">
                ...
                <div className="border border-border rounded-lg p-3 bg-card space-y-2">
                  <div className="text-xs font-semibold text-primary font-tamil">தொகை விவரம்</div>

                  <div>
                    <Label className="text-xs font-tamil">மொத்த கிரையத் தொகை / Total Amount (₹)</Label>
                    <Input
                      type="number"
                      value={totalAmount || ''}
                      onChange={e => {
                        ...
                        setTotalAmount(v);
                        setBalanceAmount(Math.max(0, v - advanceAmount));
                      }}
                      className="h-8 text-sm font-english"
                    />
                    {totalWords && <p className="text-xs text-primary font-tamil mt-1 bg-primary/5 rounded px-2 py-1">{totalWords}</p>}
                  </div>

                  <div>
                    <Label className="text-xs font-tamil">முன்பணம் / Advance Amount (₹)</Label>
                    <Input
                      type="number"
                      value={advanceAmount || ''}
                      onChange={e => {
                        const v = Number(e.target.value);
                        setAdvanceAmount(v);
                        setBalanceAmount(Math.max(0, totalAmount - v));
                      }}
                      className="h-8 text-sm font-english"
                    />
                    {advanceWords && <p className="text-xs text-primary font-tamil mt-1 bg-primary/5 rounded px-2 py-1">{advanceWords}</p>}
                  </div>

                  <div>
                    <Label className="text-xs font-tamil">பாக்கி தொகை / Balance Amount (₹)</Label>
                    <Input
                      type="number"
                      value={balanceAmount || ''}
                      ...
                      className="h-8 text-sm font-english"
                    />
                    {balanceWords && <p className="text-xs text-primary font-tamil mt-1 bg-primary/5 rounded px-2 py-1">{balanceWords}</p>}
                  </div>

                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <Label className="text-xs font-tamil">கெடு காலம் / Deadline</Label>
                      <Input
                        value={deadline}
                    ...
                    <div>
                      <Label className="text-xs font-tamil">அலகு / Unit</Label>
                      <Select value={deadlineUnit} onValueChange={setDeadlineUnit}>
                        <SelectTrigger className="h-8 text-sm font-tamil"><SelectValue /></SelectTrigger>
                        <SelectContent>{DEADLINE_UNITS.map(u => <SelectItem key={u} value={u} className="font-tamil">{u}</SelectItem>)}</SelectContent>
                      </Select>
                    </div>
                  </div>
                </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-border bg-card no-print">
          ...
          <Button variant="outline" onClick={handleSaveDraft} disabled={saveDraft.isPending} className="gap-1 font-tamil text-sm">
      ...
      <div className="w-1/2 flex flex-col">
        <TamilPreview content={previewText} title="கிரைய உடன்படிக்கை பத்திரம்" />
      </div>
    </div>
  );
}
